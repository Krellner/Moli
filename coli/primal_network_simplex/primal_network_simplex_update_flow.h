...
template <typename I,
          typename V>
inline void updateFlow(const V delta,                                   //
                       const I apex,                                    //ˇ
...
)
{
    ...
    // edge via the apex to the head of the entering edge. If the flow on the entering edge was zero, 
    // the flow is pushed in the opesite direction, i.e. from the head to the tail of the entering edge.
...
{
    logger.start();
    updateFlow<I, V>(delta, apex, entering_idx, entering_v, entering_w, predecessor, parent,
