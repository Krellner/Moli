...
)
{
    ...
    vector<I> predecessor(nVertices + 1, nEdges + nVertices); // for each vertex the incoming tree edge
    ...
    vector<Direction> direction_predecessor(nVertices + 1);   // for each vertex the orientation of its predecessor
    vector<State> state(nEdges, LOWER);                       // for each edge it status with in the spanning tree

    initialize<I, V, Logger>(
    ...
    while (true)
    {
        ...
        pivot<I, V, Logger, Rule>(
            block_size, nEdges, capacity, flow, supply, costs, potentials, number_successors, parent, predecessor, source,
            target, direction_predecessor, state, change, delta, apex, length, entering_edge, in_v, in_w, out_v,
            leaving_edge, start_edge, logger);

        if (entering_edge == nEdges)
            break;

        update<I, V, Logger>(
            change, delta, apex, entering_edge, source[entering_edge], target[entering_edge], leaving_edge,
            length, in_v, in_w, out_v, costs, potentials, flow, last_successor, number_successors, parent,
            predecessor, reversed_thread, thread, direction_predecessor, state, logger);
    }
}