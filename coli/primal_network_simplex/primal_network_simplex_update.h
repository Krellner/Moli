...
{
    if (delta > 0)
        updateFlow<I, V, Logger>(state[entering_idx] * delta, apex, entering_idx, entering_v,
                                 entering_w, predecessor, parent, direction_predecessor, flow,
                                 logger);

    updateState<I, V, Logger>(entering_idx, leaving_idx, change, flow, state, logger);

    // if change_type == 0 (i.e. false) then entering edge is leaving edge, nothing needs to be done
    if (change)
    {
        updateTree<I, Logger>(apex, in_v, in_w, length, entering_idx, entering_v, entering_w, out_v,
                              last_successor, number_successors, parent, predecessor,
                              reversed_thread, thread, direction_predecessor, logger);

        V oriented_costs = direction_predecessor[in_v] * costs[entering_idx];
        updatePotentials<I, V, Logger>(last_successor[in_v], in_v, in_w, oriented_costs,
                                       number_successors[in_v], thread, reversed_thread, potentials,
                                       logger);
    }
}